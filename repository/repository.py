...
class Repository(object):
    ...
    def search_products(self, product, fuzziness):
        ...
        try:
            result = self.__cluster.search_query(
                'basic-search',
                TermQuery(term=product, fuzziness=fuzziness) if fuzziness else TermQuery(
                    term=product),
    ...
    def get_order(self, order_id):
        ...
        '''
        try:
            result = self.__collection.get(order_id)
            return {
                'order': result.content if result is not None and result.content is not None else None,
                'error': None
            }
        except Exception as ex:
            output_message(
    ...
    def save_order(self, order):
        ...
        try:
            ...
            key = 'order_{0}'.format(order_id+1)

            order['_id'] = key
            order['orderId'] = order_id+1
            order['doc']['created'] = int(datetime.datetime.now().timestamp())
            order['doc']['createdBy'] = order['custId']
            saved = self.__collection.insert(key, order)
            if not saved:
                return {
                    'order': None,
            ...
            saved_order = self.get_object_by_key(key)
            return {
                'order': saved_order['result'],
                'error': None
            }
        except Exception as ex:
            output_message(
    ...
    def replace_order(self, order):
        ...
        '''
        try:
            key = 'order_{0}'.format(order['orderId'])
            order['doc']['modified'] = int(datetime.datetime.now().timestamp())
            order['doc']['modifiedBy'] = order['custId']
            saved = self.__collection.replace(key, order)
            return {
    ...
    def delete_order(self, order_id):
        ...
        '''
        try:
            result = self.__collection.remove(order_id)
            return {
    ...
    def save_address(self, cust_id, path, address):
        ...
        try:
            ...
            result = self.__collection.lookup_in(key, [
                #SD.exists(path),
                SD.get(path)
            ])
